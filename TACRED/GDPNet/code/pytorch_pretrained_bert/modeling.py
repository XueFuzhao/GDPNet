...
class OutputAttention(nn.Module):
    ...
    def forward(self, context, mask=None):
        ...
        before_combine = attention_weights*context
        combined = before_combine.sum(1)
...
class GGG(nn.Module):
    def __init__(self, config, args):
        ...
        self.transform_2 = nn.Linear(self.hid_dim, self.hid_dim)
    ...
    def forward(self, x, adj, mask = None):
        ...
        if mask is None:
            x_new = self.pool_X(x)
            x_new = torch.cat([x_new,x],dim=2)

            x_new = self.transform(x_new)
        else:
            x_new = self.transform_2(x)
        ...
        if mask is not None:
            ...
            for i in range(len(attn_adj_list)):
                 attn_adj_list[i] = attn_adj_list[i].masked_fill(mask_matrix == 0, 0)
...
class PoolGCN(nn.Module):
    ...
    def forward(self, adj, inputs, input_id):
        ...
        _, attn_adj_list = self.GGG(gcn_inputs,adj)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self,args, config, num_labels):
        ...
        self.classifier = nn.Linear(config.hidden_size + args.graph_hidden_size, num_labels)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        # h_out = self.output_attention(h, pool_mask)
        ...
        if labels is not None:
            ...
            src_mask_input = src_mask_input.reshape(
                [src_mask_input.size(0), src_mask_input.size(2), src_mask_input.size(1)])
            ...
            #print(DTW_hidden_0.size())
            #print(DTW_hidden_1.size())
