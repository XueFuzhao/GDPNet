...
def main(args):
    ...
    if args.do_train:
        ...
        for lr in lrs:
            model = BertForSequenceClassification.from_pretrained(args,
                args.model, cache_dir=str(PYTORCH_PRETRAINED_BERT_CACHE), num_labels=num_labels)
                #args.model,  cache_dir=str(PYTORCH_PRETRAINED_BERT_CACHE), num_labels=num_labels, args=args)
            #if args.fp16:
            #    model.half()
            model.to(device)
            if n_gpu > 1:
    ...
    if args.do_eval:
        if args.eval_test:
            ...
            eval_dataloader = DataLoader(eval_data, batch_size=args.eval_batch_size)
            eval_label_ids = all_label_ids
        model = BertForSequenceClassification.from_pretrained(args, args.output_dir, num_labels=num_labels)
        #if args.fp16:
        #    model.half()
        model.to(device)
        preds, result = evaluate(model, device, eval_dataloader, eval_label_ids, num_labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lamada', type=float, default=0.0002, help='Weights for DTW Loss.')
